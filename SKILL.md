...
目标：拿到一份 WeFlow 微信导出（jsonl + media），产出可部署的回忆网站（卫星地图 + 时间线 + 相册 + 语音 + 聊天人设模块），并训练对方的语音模型。
...
## 4. 网页
- 地图：Leaflet 加合规底图瓦片（来源与署名由使用者自行解决）；坐标优先用聊天 [位置] 消息，自标点注明"坐标约"；点与点之间细线连接；默认视野框住核心区域。
...
- 入口页：默认不包含入口动画页；站点入口由使用者自行指定（GitHub Pages 如要求默认页，命名 main.html 为 index.html 即可）。

## 5. 部署检查清单
1. 删除或替换所有真实 API Key（聊天模块从 localStorage 读）。
2. 隐私：私聊原数据（media 和 jsonl）不要上传公开仓库，照片只用精选集。
3. GitHub Pages：文件夹结构就是站点根，入口以 main.html 为准（平台要求默认页时可改名为 index.html）。
